...
////////////////////////////////////////////////////////////////////////////////////////////////////
  try{
    for(unsigned int thrd=0; thrd<feconfigs.size(); thrd++){
      conf_threads.push_back(std::thread(&nsw::THRCalib::sca_calib, this, thrd));
    }
  }catch(std::exception &e){
     std::string errmsg = e.what();
     throw std::runtime_error("THRCalib::configure::ERROR - "+errmsg);
...
void nsw::THRCalib::read_config(std::string config_filename)
{ 
 ...
 //----------- pushing set into vector to sort FEB names by their serial numbers -------------------------
  fe_names_v.assign(fenames.begin(), fenames.end());
...
std::vector<float> nsw::THRCalib::read_baseline(
    nsw::ConfigSender &cs,
    nsw::FEBConfig &feb,
...
int nsw::THRCalib::calculate_thdac_value(nsw::ConfigSender & cs,
                          nsw::FEBConfig & feb,
             nsw::CalibrationMath &cm,
                          int vmm_id,
                          int n_samples,
...
std::pair<float,int> nsw::THRCalib::find_linear_region_slope(nsw::ConfigSender & cs,
                            nsw::FEBConfig & feb,
                        nsw::CalibrationMath &cm,
                   int vmm_id,
                            int channel_id,
...
std::vector<float> nsw::THRCalib::vmm_averages
 (nsw::ConfigSender &cs,
 nsw::FEBConfig &feb,
 int i_vmm,
 int channel_id,
...
std::map< std::pair< std::string,int>, int>  nsw::THRCalib::analyse_trimmers
   (nsw::ConfigSender &cs,
   nsw::FEBConfig &feb,
   nsw::CalibrationMath &cm,
   int i_vmm,
...
void nsw::THRCalib::sca_calib(int fe_name_sorted)
{
...
//----------------------------------------------------------------
 ...
 auto feb = feconfigs.at(fe_name_sorted);
 std::string fe_name = feb.getAddress();
...
//--------------- opening calibration data output file ------------------------------------------------------------------
 ...
 std::string full_path = out_chan_folder+fe_name+"_data.txt";
 ERS_INFO("Writing data to:("<<full_path<<")");
...
//    vmm_median = cm.cm.sample_to_mV(vmm_baseline_med[fe_name]);
    ...
    else if(ch_med_dev < -BASELINE_CUTOFF)//-cutoff)
    {
     ...
     channel_mask.push_back(1);
     dead_chan++;
     ERS_INFO("INFO: "<<fe_name <<"["<<i_vmm<<"|"<<channel_id<<"] - baseline below median >> "<<  cm.sample_to_mV(channel_baseline_med[feb_ch], stgc));
    }
    else
    {
  ...
  //---------------------------------------------------------------------------------------------------------------------------- 
      ...
      //////////////////////////////////
    ERS_INFO("Calculating "<<fe_name<<" VMM "<<i_vmm<<" global threshold"); 
      std::tuple<std::string, float, float> thdac_constants;
  ...
  //------------------------------------------------------------------------------------------------------------------------------------------------- 
    ...
    { 
     ...
     ERS_INFO("INFO: "<<fe_name<<" VMM "<<i_vmm<<" global threshold is "<< cm.sample_to_mV(mean, stgc)); 
      break;
     }
     else
     {
...
//--------- if effective thereshold on VMM level is less than 10 mV resample thresholds with trimmers and a raised THDAC ------------------------------------- 
 ...
 //--------------------------------------------------------------------------------------------------------------------------  
    ...
    if((cm.sample_to_mV(vmm_eff_thresh, stgc) >= 40) or (cm.sample_to_mV(vmm_eff_thresh, stgc) <= 5)){ //if res. effective threshold is >40 mV or <5 mV inform calibrep
     ...
     ERS_INFO("Scaning "<<fe_name<<" trimmers");
 ...
 auto lr1 = std::chrono::high_resolution_clock::now();
   ...
   ERS_INFO("INFO - {GOOD NEWS YEREVAN!}, "<<fe_name<<" VMM "<<i_vmm<<" -> "<< good_chs << " out of " << tot_chs << " are okay!");
...
//--------------- channel calib parameters to file -----------------------------------     
  ...
  auto f0 = std::chrono::high_resolution_clock::now();
   ...
   ERS_INFO("INFO: "<<fe_name<<" VMM "<<i_vmm<<"Trimmers calculated/data written");
...
//--------------------end of board configuration clearing containers to save memory ---------------------------- 
 ...
 ERS_INFO("INFO: Thresholds calibrated");

}
std::vector<short unsigned int> nsw::THRCalib::ch_threshold(nsw::ConfigSender &cs,
                  nsw::FEBConfig &feb,
                  int vmm_id,
                  int channel_id,
                  int n_samples)
...
void nsw::THRCalib::read_thresholds(
...
//-------------------------------------------------------------
 ...
 auto feb = feconfigs.at(fe_name_sorted);
 std::string fe_name = feb.getAddress();
 ERS_INFO("Starting to read "<<fe_name);
 namespace pt = boost::property_tree;
 pt::ptree input_data;
 pt::read_json(io_config_path,input_data);
 ...
 ERS_INFO("Writing thresholds to "<< path2);
...
//--------------------- text file output ----------------------------------------------------------------------------------------------
  ...
  if(debug){ERS_INFO(" VMM_"<<vmm_id<<" thresholds done");}
   }//vmm loop ends
...
void nsw::THRCalib::read_baseline_full(
           std::string io_config_path,
           int n_samples,
           int fe_name_sorted,
           bool conn_check)
...
////////////////////////////////// pulser calibration ////////////////////////////////////////////////////////////////////////
 ...
 void nsw::THRCalib::calib_pulserDAC( 
                       std::string io_config_path,
                       int n_samples,
                       int fe_name_sorted,
                       bool debug)
