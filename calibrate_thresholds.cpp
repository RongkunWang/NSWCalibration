...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////// !! PART WITH FUNCTION DEFINITIONS !! /////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//-------------- functions for calibrate_trimmers.cpp--------------

int NCH_PER_VMM = 64; //10//64; //10;//64;
float RMS_CUTOFF = 30; // in mV (or \approx 80ADC counts) //original is 30 mVi
float BASELINE_CUTOFF = 20; // in mV Using a bit higher cutoff
int TRIM_HI = 31;
int TRIM_LO = 0;
int TRIM_MID = 14;
...
//-------------- trying out inline func. -------------------
inline float inl_sample_to_mV(short unsigned int sample){
  return sample * 1000. * 1.5 / 4095.0; //1.5 is due to a resistor
}
//-----------------------------------------------------------
													...
													bool debug)
{
	  ...
	  for (unsigned int i = 0; i < thdac_guess_variations.size(); i++)
	    ...
	    auto all_results = cs.readVmmPdoConsecutiveSamples(feb, vmm_id, n_samples*2);
			...
			float thdac_med = take_median(all_results);
			float thdac_diff;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////// BASELINE AND OTHER READOUT FUNCTIONS /////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////

std::vector<float> read_baseline
				(nsw::ConfigSender &cs,
				nsw::FEBConfig &feb, 
				int i_vmm,
				int channel_id,
...
//--------------------- vmm average level calc function -----------------------------------------
//
std::vector<float> vmm_averages
	(nsw::ConfigSender &cs,
	nsw::FEBConfig &feb,
	int i_vmm,
	int channel_id,
...
//
//--------------------- trimmer analysis function -----------------------------------------

std::map< std::pair< std::string,int>, int>  analyse_trimmers
			(nsw::ConfigSender &cs,
			nsw::FEBConfig &feb,
			int i_vmm,
			int channel_id,
...
//-------------- baseline loop ends here -------------------------------------------------------
	    ...
	    float tmp_median = take_median(fe_samples_tmp); // this thing raises the vmm average level ... i think ...																									//-----------------------------------
//-------------------------------------------	
			bool bad_bl = false;
...
//				vmm_median = inl_sample_to_mV(vmm_baseline_med[fe_name]); 
				ch_med_dev = inl_sample_to_mV(channel_baseline_med[feb_ch]) - inl_sample_to_mV(tmp_median);//vmm_baseline_med[feb.getAddress()]);
				ch_noise = inl_sample_to_mV(channel_baseline_rms[feb_ch]); //checking the noise
				sum_rms += ch_noise; 
	    ...
	    for (auto sample: fe_samples_tmp)
		      ...
		      if ((channel_mask.at(CH)!=1) and fabs(sample_to_mV(sample - tmp_median)) < RMS_CUTOFF)/*mV*/ // kicking out the samples with 30mV above median, with channel check
					{
		    ...
		    float vmm_median = take_median(fe_samples_pruned);
				...
				" : Calculated global {tmp_vmm_median = "<<inl_sample_to_mV(tmp_median)<<"}{vmm_median = "<<inl_sample_to_mV(vmm_median)<<"}{vmm_stdev = "<<inl_sample_to_mV(vmm_stdev)<<"}"<<std::endl;}
		...
		//----------------------------------------------------------------------------------------------------------------------------	
	    	...
	    	int thdac_central_guess = rms_factor * sample_to_mV(vmm_baseline_rms[fe_name]) + sample_to_mV(vmm_baseline_med[fe_name]) + offset_center;
		    ...
		    std::cout << "\nINFO - "<<fe_name<<" VMM_"<<i_vmm<<" baseline_mean, baseline_med, baseline_rms, rms_factor: "
		                ...
		                << sample_to_mV(vmm_baseline_med[fe_name]) << ", "
		                << sample_to_mV(vmm_baseline_rms[fe_name]) << ", "
		                << rms_factor
		                << std::endl;
		//------------------ against this value later calibration result is checked to see if the result is what was expected ------------------------------
				float expected_eff_thr = inl_sample_to_mV(vmm_baseline_rms[fe_name] * (rms_factor));
		//-------------------------------------------------------------------------------------------------------------------------------------------------	
		    ...
		    std::vector<int> thdac_guess_variations = {100,150,200,250,300,350,400};
				...
				{	
			    thdac = calculate_thdac_value(cs,feb,i_vmm,n_samples,thdac_central_guess,thdac_constants,thdac_guess_variations, debug);
		  		...
		  		auto results = cs.readVmmPdoConsecutiveSamples(feb, i_vmm, n_samples);
		   		...
		   		mean = sum / results.size();
					float thr_diff = mean - inl_sample_to_mV(vmm_baseline_med[fe_name]);
					if(thr_diff <= 0)
					{
		   		...
		   		thdacs_sample[feb.getAddress()] = mean;  //actually set threshold, previous samples were readout on guess
					float thdac_dev = fabs(inl_sample_to_mV(mean) - thdac_central_guess)/thdac_central_guess;
					if(thdac_dev < 0.1)
					{
			 			std::cout << "\nINFO - Threshold for " << feb.getAddress() << " vmm" << i_vmm << " is " << sample_to_mV(mean) << " in mV with deviation from guess: ["<<thdac_dev*100<<" %]"<<std::endl;
						break;
					}
					else
					{
						std::cout<<"\nWARNING - "<<feb.getAddress()<<"VMM_"<<i_vmm<<" - {"<<thdac_dev*100<<"%} deviation in threshold calculation\n"<<"[guess = "<<thdac_central_guess<<" -> calc. = "<<inl_sample_to_mV(mean)<<"] : repeating calculation "<<th_try<<" time"<<std::endl;	
					std::this_thread::sleep_for(std::chrono::milliseconds(40));
					}		
		  	}
			  thdacs[feb.getAddress()] = thdac;			
				if((mean - inl_sample_to_mV(vmm_baseline_med[fe_name])) < 0){
					mtx.lock();
...
//--------- if effective thereshold on VMM level is less than 10 mV resample thresholds with trimmers and a raised THDAC -------------------------------------	
				if(vmm_eff_thresh < 30)// 30 ADC counts - roughly 10 mV;
			 	{
					fe_samples_tmp.clear();
					int noise_fact = inl_sample_to_mV(vmm_baseline_rms[feb.getAddress()]);	//converting noise rms adc to mV
	...
	//--------------------------------------------------------------------------------------------------------------------------		
		    ...
		    vmm_mid_eff_thresh[feb.getAddress()] = vmm_eff_thresh;   /// fill the mapo with median value with trimmers enabled at the middle value
				...
				if((inl_sample_to_mV(vmm_eff_thresh) >= 40) or (inl_sample_to_mV(vmm_eff_thresh) <= 5)){ //if res. effective threshold is >40 mV or <5 mV inform calibrep
					mtx.lock();
					calibrep<<fe_name<<" VMM_"<<i_vmm<<" high threshold [result:"<<inl_sample_to_mV(vmm_eff_thresh)<<"(mV), expected:"<<expected_eff_thr<<"(mV), noise:"<<inl_sample_to_mV(vmm_stdev)<<"(mV)]\n"<<std::endl;	
					mtx.unlock();
				}
		    //////////////////////////////////
		    //////////////////////////////////
		    ...
		    for (int channel_id = 0; channel_id < NCH_PER_VMM; channel_id++){
		      ...
		      if (!check_channel(ch_baseline_med, ch_baseline_rms, vmm_baseline_med[fe_name])){continue;}
	...
	float trim_rms = take_rms(trim_perf, trim_mean);																									// check for eff. thr. scattering around mean
	...
	for(int i=0; i<NCH_PER_VMM; i++)
	{
		...
		if(extraDAC > 0 and inl_sample_to_mV(extraDAC) <= 16)/*mV, half of trimmer working range*/
		{		
	...
	if(add_dac.size()>0)
	{
	  ...
	  for (int channel_id = 0; channel_id < NCH_PER_VMM; channel_id++)
		...
		float trim_mean = (std::accumulate(trim_perf.begin(), trim_perf.end(), 0.0)/(trim_perf.size()));
		float trim_rms2 = take_rms(trim_perf, trim_mean);
		...
		auto f0 = std::chrono::high_resolution_clock::now();
			for(int channel_id=0;channel_id<NCH_PER_VMM; channel_id++)
			{
				...
				ch_calib_data<<feb.getAddress()<<"\t"
							<<i_vmm<<"\t"
							<<channel_id<<"\t"
							<<sample_to_mV(channel_baseline_med[feb_ch])<<"\t"			
							<<sample_to_mV(channel_baseline_rms[feb_ch])<<"\t"
							<<sample_to_mV(channel_mid_eff_thresh[feb_ch])<<"\t"
...
//------------ Channel threshold reading functions ---------------------------------------------------------------------
    ...
    for (int vmm_id = 0; vmm_id < n_vmms; vmm_id++) {
      ...
      for (int channel_id = 0; channel_id < 64; channel_id++) {
    	  	  ...
    	  	  float mean   = sum / results.size();	
						...
						if(mean < 100)
						{
							std::cout<<"WARNING - "<<feb.getAddress()<<" VMM_"<<vmm_id<<" channel - "<<channel_id<<" -> threshold might be dead = ["<<inl_sample_to_mV(mean)<<" mV]"<<std::endl;
						}
...
//-------------- searching for max and min deviation in samples -----------------------------------------------------------	   
					  ...
					  float min_dev;
						...
						if(thr_dev > 50)
						{
							std::cout<<feb.getAddress()<<" VMM_"<<vmm_id<<" channel - "<<channel_id<<" -> high threshold deviation = ["<<inl_sample_to_mV(thr_dev)<<" mV]"<<std::endl;
							dev_thr++;
						}
					
//--------------------- text file output ----------------------------------------------------------------------------------------------
						if(debug){std::cout<<feb.getAddress()<<"\t"<<vmm_id<<"\t"<<channel_id<<"\t"<<sample_to_mV(mean)<<"\t"<<sample_to_mV(max_dev)<<"\t"<<sample_to_mV(min_dev)<<std::endl;}  
						thr_test<<feb.getAddress()<<"\t"<<vmm_id<<"\t"<<channel_id<<"\t"<<sample_to_mV(mean)<<"\t"<<sample_to_mV(max_dev)<<"\t"<<sample_to_mV(min_dev)<<std::endl;  
...
///////////////////////////////////// reading all samples for baseline /////////////////////////////////////////////////////////////////////
  	...
  	for (int vmm_id = 0; vmm_id < n_vmms; vmm_id++) {
    	...
    	for (int channel_id = 0; channel_id < 64; channel_id++) {
    		...
    		auto results = cs.readVmmPdoConsecutiveSamples(feb, vmm_id, n_samples*10);
				...
				{
					full_bl<<fe_name<<"\t"<<vmm_id<<"\t"<<channel_id<<"\t"<<inl_sample_to_mV(result)<<std::endl;  
				}
				float sum    = std::accumulate(results.begin(), results.end(), 0.0);
     	  float mean   = sum / results.size();	
				float median = take_median(results); 
				float rms = take_rms(results,mean); 
	
				if(conn_check)
				{
					if(inl_sample_to_mV(mean)>180)
					{
						fault_chan++;
						printf("\nWARNING - %s - VMM %i - %i high baseline = <<%f mV>>, probably needs to be masked\n", fe_name.c_str(), vmm_id, channel_id, inl_sample_to_mV(mean));
					}
					if(inl_sample_to_mV(mean)<140)
					{
						fault_chan++;
...
//-------------- searching for max and min deviation in samples -----------------------------------------------------------	   
	   	  ...
	   	  if(conn_check)
				{
					std::cout<<"INFO - "<<fe_name<<" VMM_"<<vmm_id<<" CH:"<<channel_id<<" - BL: "<<inl_sample_to_mV(median)<<" - RMS: "<<inl_sample_to_mV(rms)<<", spread: "<<inl_sample_to_mV(sample_dev)<<std::endl;
				//	if(inl_sample_to_mV(sample_dev)>60)
				//	{
			...
			try{
				...
				//		std::cout<<fe_names_v[l]<<std::endl;
					...
					for(long unsigned int b=0; b<fe_names_v.size(); b++)
					{
						...
						{
							conf_threads[ifeb] = std::thread(configure_feb, frontend_configs, b);
							ifeb++;
						}
						else{continue;}
					}
...
//----------------------------further is executed if no specific board set is specified-----------------------------------------------------------------------------------------
		else
		{
			try{	
				...
				{
					conf_threads[i] = std::thread(configure_feb, frontend_configs, i);
				}
		...
		{	
			...
			//---------------- threshold reading here ---------------------------------
				...
				for(unsigned int l=0; l<fe_names_v.size(); l++)
				{
					if(fe_names_v[l].find(dw_layer)!=std::string::npos)
					{
						if(baseline){conf_threads[ifeb] = std::thread(read_baseline_full, config_filename, frontend_configs, io_config_path, n_samples, l, pFEB, fe_names_v.at(l),conn_check);}
						if(threshold){conf_threads[ifeb] = std::thread(read_thresholds, config_filename, frontend_configs, io_config_path, n_samples, l, pFEB, debug, fe_names_v.at(l));}
						ifeb++;
					}
					else{continue;}
...
//----------------------------further is executed if no specific board set is specified-----------------------------------------------------------------------------------------
		else
		{
				...
				try{
					...
					for(int i=0; i<N_FEB; i++)
					{
						if(threshold){conf_threads[i] = std::thread(read_thresholds, config_filename, frontend_configs, io_config_path, n_samples, i, pFEB, debug, *ct_it);}
						if(baseline){conf_threads[i] = std::thread(read_baseline_full, config_filename, frontend_configs, io_config_path, n_samples, i, pFEB, *ct_it, conn_check);}
						ct_it++;
					}
		...
		if(dw_layer.length()>0)
		{
			...
			try{
				for(long unsigned int b = 0; b < fe_names_v.size(); b++)
				{
					...
					{
						conf_threads[ifeb] = std::thread(sca_calib, config_filename, frontend_configs, fe_names_v[b], io_config_path, b, n_samples, pFEB, debug, rms);
						ifeb++;
					}
					else{continue;}
				}
			...
			std::this_thread::sleep_for(std::chrono::seconds(1));
			if(nfebs>1){merge_json(mod_json, io_config_path);} //generate new .json file 
			}
			catch(std::exception & e)
			{
...
//--------------- in case of need to configure whole set of boards -----------------------------
		else
		{
			...
			try{				
				...
				for(int i = 0; i < N_FEB; i++)
				{
					conf_threads[i] = std::thread(sca_calib, config_filename, frontend_configs, *ct_it, io_config_path, i, n_samples, pFEB, debug, rms);
					ct_it++;
				}
				for(int j=0;j<N_FEB;j++)
				{
				...
				printf("\nthreads joined!\n");
			std::this_thread::sleep_for(std::chrono::seconds(1));
			if(N_FEB>1){merge_json(mod_json, io_config_path);} //generate new .json file 
		}
		catch(std::exception &e){
				std::cout<<"Error on thread: ["<<e.what()<<"]"<<std::endl;
				calibrep<<"\nERROR interupt: "<<e.what()<<std::endl;}
...
//================== Merging separate board configs ===============================================
	...
	if(merge_config){
		...
		try{
			merge_json(mod_json, io_config_path);
		}catch(std::exception &e){
